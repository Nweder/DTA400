...
#     plt.title(f"{dataset_name}: eps={eps:.3f}, min_samples={min_samples}")
#     plt.xlabel("X"); plt.ylabel("Y")
#     fn = f"plots/{dataset_name}_clusters_eps{eps:.3f}_m{min_samples}.png".replace('.', '_')
#     plt.savefig(fn, dpi=300)
#     plt.close()

# def kdist_elbow(X2d, dataset_name, k=4):
#     """Valfritt: elbow-kurva för eps-gissning."""
...
# def simulate(dataset_path, eps_values, min_samples_values, max_plots=6):
#     ensure_dirs()
#     name = os.path.splitext(os.path.basename(dataset_path))[0]

#     # 1) Ladda
#     pcd = np.load(dataset_path)  # form: (N,3) => X,Y,Z
...
#             # spara några representativa klusterbilder
#             if plot_count < max_plots:
#                 plot_clusters(XY, labels, name, eps, m)
#                 plot_count += 1

#             print(f"[{name}] eps={eps:.3f} m={m} -> clusters={n_clusters} noise={noise_ratio:.3f} largest={largest}")

#     rows = np.array(rows, dtype=float)
#     np.savetxt(f"results/{name}_grid.csv",
#                rows,
#                delimiter=",",
#                header="eps,min_samples,n_clusters,noise_ratio,largest_cluster",
#                comments="")

#     # 5) Sammanfattningsgrafer vs eps (fixera min_samples till min value för översikt)
#     m0 = min(min_samples_values)
#     mask = rows[:,1] == m0
#     r = rows[mask]
#     # sort per eps
#     idx = np.argsort(r[:,0])
...
#     plt.title(f"{name}: clusters vs eps (min_samples={int(m0)})")
#     plt.savefig(f"plots/{name}_clusters_vs_eps_m{int(m0)}.png", dpi=300)
#     plt.close()
...
#     plt.title(f"{name}: noise vs eps (min_samples={int(m0)})")
#     plt.savefig(f"plots/{name}_noise_vs_eps_m{int(m0)}.png", dpi=300)
#     plt.close()

#     # Största kluster vs eps
#     plt.figure()
#     plt.plot(r[:,0], r[:,4], marker='o')
#     plt.xlabel("eps"); plt.ylabel("largest cluster size")
#     plt.title(f"{name}: largest cluster vs eps (min_samples={int(m0)})")
#     plt.savefig(f"plots/{name}_largest_vs_eps_m{int(m0)}.png", dpi=300)
#     plt.close()

#     print(f"[{name}] Saved: results/{name}_grid.csv and plots/*.png")

# if __name__ == "__main__":
#     if len(sys.argv) < 2:
#         print("Usage: python simulation.py <dataset.npy> [eps_start eps_stop eps_step [min_s_min min_s_max]]")
#         sys.exit(1)

#     ds = sys.argv[1]
#     # standardintervall som funkar för dina data
#     eps_start, eps_stop, eps_step = (0.05, 0.25, 0.025) if len(sys.argv) < 5 else map(float, sys.argv[2:5])
#     ms_min, ms_max = (3, 7) if len(sys.argv) < 7 else map(int, sys.argv[5:7])

#     eps_values = np.arange(eps_start, eps_stop + 1e-9, eps_step)
#     min_samples_values = list(range(ms_min, ms_max + 1))
#     simulate(ds, eps_values, min_samples_values)
...
# -*- coding: utf-8 -*-
"""
DBSCAN grid-svep + Monte Carlo-simulering för robusthetsanalys.
- Läser .npy med kolumner [X,Y,Z].
- Detekterar marknivå via Z-histogram.
- Filtrerar punkter ovan mark och kör:
  1) Grid-svep över (eps, min_samples)  -> plots + CSV
  2) Monte Carlo m. bootstrap + priors  -> plots + CSV + JSON-sammanfattning

Kör:
python simulation_mc.py <dataset.npy> [N_mc] [eps_start eps_stop eps_step] [min_s_min min_s_max] [subsample_frac] [seed]

Exempel:
python simulation_mc.py data.npy 3000 0.05 0.25 0.025 3 7 0.8 42
"""
import os
import sys
import json
