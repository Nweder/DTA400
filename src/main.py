...
matplotlib.use("Agg")  # viktigt för att spara figurer

def ensure_dirs():
    os.makedirs("plots", exist_ok=True)
    os.makedirs("results", exist_ok=True)
    print("[INFO] Saving figures to:", os.path.abspath("plots"))
...
def kdist_elbow(X2d, dataset_name, k=4):
    """Elbow-curve for eps estimation based on k-nearest neighbour distances."""
    nn = NearestNeighbors(n_neighbors=k).fit(X2d)
    dists, _ = nn.kneighbors(X2d)
    d = np.sort(dists[:, k-1])
    ...
    plt.figure()
    plt.plot(d)
    plt.title(f"{dataset_name}: k-distance curve (k={k})")
    plt.xlabel("Points sorted by distance rank")
    plt.ylabel(f"{k}-nearest neighbour distance")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"plots/{dataset_name}_kdist_k{k}.png", dpi=300)
    plt.close()
...
def grid_sweep(XY, eps_values, min_samples_values, dataset_name, max_plots=6):
    ...
    for m in min_samples_values:
        for eps in eps_values:
            ...
            print(
                f"[GRID {dataset_name}] eps={eps:.3f} m={int(m)} "
                f"-> clusters={n_clusters} noise={noise_ratio:.3f} largest={largest}"
            )
    ...
    # largest cluster vs eps
    plt.figure()
    plt.plot(r[:, 0], r[:, 4], marker='o')
    plt.xlabel("eps")
...
def monte_carlo(
    ...
    # Histogram: number of clusters
    print("\n[FIGURE 1] Histogram of cluster counts")
    print("   X-axis: Number of clusters (dimensionless)")
    print("   Y-axis: Relative frequency (probability density)\n")
    plt.figure()
    plt.hist(ncl, bins=40, density=True, label="Histogram")
    plt.title(f"{dataset_name} Monte Carlo: distribution of number of clusters")
    plt.xlabel("Number of clusters")
    plt.ylabel("Relative frequency")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"plots/{dataset_name}_mc_hist_nclusters.png", dpi=300)
    plt.close()

    # ECDF: number of clusters
    print("[FIGURE 3] Empirical cumulative distribution (ECDF) of cluster count")
    print("   X-axis: Cluster count (dimensionless)")
    print("   Y-axis: Cumulative probability F(k) (0–1)\n")
    y_ecdf, x_ecdf = ecdf(ncl)
    plt.figure()
    plt.plot(y_ecdf, x_ecdf)
    plt.title(f"{dataset_name} Monte Carlo: empirical CDF of cluster count")
    plt.xlabel("Number of clusters")
    plt.ylabel("Empirical CDF F(k)")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"plots/{dataset_name}_mc_ecdf_nclusters.png", dpi=300)
    plt.close()

    # Histogram: noise ratio
    print("[FIGURE 5] Histogram of noise ratio")
    print("   X-axis: Noise ratio (fraction of total points)")
    print("   Y-axis: Relative frequency\n")
    plt.figure()
    plt.hist(noi, bins=40, density=True, label="Histogram")
    plt.title(f"{dataset_name} Monte Carlo: distribution of noise ratio")
    plt.xlabel("Noise ratio (fraction of points)")
    plt.ylabel("Relative frequency")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"plots/{dataset_name}_mc_hist_noise.png", dpi=300)
    plt.close()

    # Histogram: largest cluster size
    print("[FIGURE 6] Histogram of largest cluster size")
    print("   X-axis: Largest cluster size (number of points)")
    print("   Y-axis: Number of iterations\n")
    plt.figure()
    plt.hist(lar, bins=40, density=False, label="Histogram")
    plt.title(f"{dataset_name} Monte Carlo: distribution of largest cluster size")
    plt.xlabel("Size of largest cluster (number of points)")
    plt.ylabel("Number of iterations")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"plots/{dataset_name}_mc_hist_largest.png", dpi=300)
    plt.close()

    # Convergence of 95th percentile of cluster count
    print("[FIGURE 2] Convergence of 95th percentile of cluster count")
    print("   X-axis: Number of Monte Carlo iterations (runs)")
    print("   Y-axis: 95th-percentile cluster count (dimensionless)\n")
    steps = np.linspace(100, N, 40, dtype=int)
    p95_run = [np.percentile(ncl[:k], 95) for k in steps]
    plt.figure()
    plt.plot(steps, p95_run)
    plt.title(f"{dataset_name} Monte Carlo: convergence of 95th percentile of cluster count")
    plt.xlabel("Number of Monte Carlo iterations")
    plt.ylabel("95th percentile of cluster count")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"plots/{dataset_name}_mc_convergence_p95_nclusters.png", dpi=300)
    plt.close()

    # Tornado-like bar plot of |Spearman| sensitivities
    print("[FIGURE 4] Sensitivity analysis using Spearman rank correlation")
    print("   X-axis: |Spearman rank correlation| (0–1, unitless)")
    print("   Y-axis: Parameter–output pairs\n")
    keys = list(sens.keys())
    vals = [abs(sens[k]) for k in keys]
    idx = np.argsort(vals)[::-1]
    keys_sorted = [keys[i] for i in idx]
    ...
    plt.barh(range(len(vals_sorted)), vals_sorted)
    plt.yticks(range(len(vals_sorted)), keys_sorted)
    plt.xlabel("|Spearman rank correlation|")
    plt.title(f"{dataset_name} Monte Carlo: sensitivity of outputs to eps and min_samples")
    plt.grid(True, axis="x", alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"plots/{dataset_name}_mc_sensitivity.png", dpi=300)
    plt.close()
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python simulation_mc.py <dataset.npy> [N_mc] "
            "[eps_start eps_stop eps_step] [min_s_min min_s_max] "
            "[subsample_frac] [seed]"
        )
        sys.exit(1)
    ...
    # Monte Carlo
    eps_prior = ("uniform", eps_start, eps_stop)
    min_s_prior = ("discrete_uniform", ms_min, ms_max)
